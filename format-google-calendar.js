...
var formatGoogleCalendar = (function() {
    ...
    var formatDateSameDay = function(dateStart, dateEnd, moreDaysEvent, isAllDayEvent, dayNames) {
        ...
        if (config.sameDayTimes && !moreDaysEvent && !isAllDayEvent) {
            formattedTime = '</br><h5>' + getFormattedTime(dateStart) + ' - ' + getFormattedTime(dateEnd) + '</h5>';
        }
    ...
    return {
        init: function (settingsOverride) {
            var settings = {
                calendarUrl: 'https://www.googleapis.com/calendar/v3/calendars/bkbg3tpo2hkiv0coca0fc6s3ds%40group.calendar.google.com/events?key=AIzaSyA8GUJpzCpqy_rXBs8slLThmErnqjieBn4',
                past: true,
                upcoming: true,
                sameDayTimes: true,
                dayNames: true,
